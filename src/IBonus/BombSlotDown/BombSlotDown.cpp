...
BombSlotDown::BombSlotDown(scene::ISceneManager *scenemgr, video::IVideoDriver *vidriver, scene::ISceneNode *node, core::vector3df &pos)
{
    _node = node;
    _scenemgr = scenemgr;
    _vidriver = vidriver;
    _pos = pos;
    _skin = vidriver->getTexture("./models/bonus/bomb_down.png");
...
BombSlotDown::~BombSlotDown()
{
}

void BombSlotDown::effect(std::unique_ptr<APlayer> &player)
{
    player->lessBombSlot();
}

void BombSlotDown::create()
{
...
void BombSlotDown::destroy()
{
    _mesh->remove();
}

scene::ISceneNode *BombSlotDown::getModel() const
{
    return _mesh;
}

bool BombSlotDown::isDestructible()
{
